...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // --- จัดการไฟล์รูปภาพอาหาร ---
    $image_name = $food['image']; // ใช้รูปเดิมเป็นหลัก
    if (isset($_FILES['image']) && $_FILES['image']['error'] === UPLOAD_ERR_OK) {
        ...
        // ลบรูปภาพเก่าออกถ้ามี (กรณีมาแก้ไขข้อมูล)
...
<body class="bg-light">
                    ...
                    <div class="mb-4">
                        ...
                        <?php if (!empty($food['image']) && file_exists('uploads/' . $food['image'])): ?>
                            <div class="mt-2">
                                ...
                                <img src="uploads/<?= htmlspecialchars($food['image']) ?>" width="120"
                    ...
                    <div id="recipe-container">
                        <?php if (empty($recipes)): ?>
                            <div class="row g-2 mb-2 recipe-row">
                                <div class="col-6"><input type="text" name="recipes[0][recipe_name]" class="form-control"
                                        placeholder="ชื่อวัตถุดิบ เช่น เส้นเล็ก"></div>
                                <div class="col-3"><input type="number" step="0.01" name="recipes[0][quantity]"
                                        class="form-control" placeholder="ปริมาณ เช่น 150"></div>
                                <div class="col-2"><input type="text" name="recipes[0][unit_name]" class="form-control"
                                        placeholder="หน่วย เช่น กรัม"></div>
                                <div class="col-1 text-end"><button type="button" class="btn btn-danger btn-sm w-100"
                                        onclick="this.closest('.recipe-row').remove();">X</button></div>
                            </div>
                        <?php else: ?>
                            <?php foreach ($recipes as $index => $r): ?>
                                <div class="row g-2 mb-2 recipe-row">
                                    <div class="col-6"><input type="text" name="recipes[<?= $index ?>][recipe_name]"
                                            class="form-control" value="<?= htmlspecialchars($r['recipe_name']) ?>" required>
                                    </div>
                                    <div class="col-3"><input type="number" step="0.01" name="recipes[<?= $index ?>][quantity]"
                                            class="form-control" value="<?= htmlspecialchars($r['quantity']) ?>" required></div>
                                    <div class="col-2"><input type="text" name="recipes[<?= $index ?>][unit_name]"
                                            class="form-control" value="<?= htmlspecialchars($r['unit_name']) ?>" required>
                                    </div>
                                    <div class="col-1 text-end"><button type="button" class="btn btn-danger btn-sm w-100"
                                            onclick="this.closest('.recipe-row').remove();">X</button></div>
                                </div>
    ...
    <script>
        // แปลงค่า PHP เป็นตัวเลขผ่าน parseInt เพื่อป้องกัน Syntax Error ในโปรแกรมจัดฟอร์แมตโค้ด
        let recipeIndex = parseInt('<?= count($recipes) > 0 ? count($recipes) : 1 ?>');
        ...
        document.getElementById('add-recipe-btn').addEventListener('click', function () {
            ...
            newRow.innerHTML = `
        <div class="col-6"><input type="text" name="recipes[${recipeIndex}][recipe_name]" class="form-control" placeholder="ชื่อวัตถุดิบ" required></div>
        <div class="col-3"><input type="number" step="0.01" name="recipes[${recipeIndex}][quantity]" class="form-control" placeholder="ปริมาณ" required></div>
        <div class="col-2"><input type="text" name="recipes[${recipeIndex}][unit_name]" class="form-control" placeholder="หน่วย" required></div>
        <div class="col-1 text-end"><button type="button" class="btn btn-danger btn-sm w-100" onclick="this.closest('.recipe-row').remove();">X</button></div>
    `;
